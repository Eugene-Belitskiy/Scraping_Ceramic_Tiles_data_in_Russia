...
# ════════════════════════════════════════════════════════════════════════════
# ТАБ 7 — ЦЕНОВОЕ ОКРУЖЕНИЕ (экспериментальный)
# ════════════════════════════════════════════════════════════════════════════

with tab7:
    st.subheader("Ценовое окружение КЕРАМИН")
    st.caption(
        "Экспериментальный блок. Показывает плотность конкурентных предложений "
        "вокруг каждой цены КЕРАМИН — без искажений от дальних выбросов."
    )

    if len(df_keramin) == 0:
        st.warning("КЕРАМИН не найден в выбранных фильтрах.")
    else:
        # ── Подготовка: конкурентные группы для этого таба ──────────────────
        dp7 = filtered[filtered["price"].notna()].copy()
        dp7["Группа"] = dp7.apply(
            lambda r: _assign_comp_group(r["brand"], r["country"]), axis=1
        )
        dp7_market  = dp7[dp7["Группа"] != "КЕРАМИН"]
        dp7_keramin = dp7[dp7["Группа"] == "КЕРАМИН"]

        # ── 1. Violin plot — плотность вместо ящика ─────────────────────────
        st.markdown("### Плотность цен по конкурентным группам (violin)")
        st.caption(
            "Violin показывает, где реально сосредоточена масса предложений, "
            "а не просто квартили. КЕРАМИН — красные точки."
        )

        fmt_opts7 = sorted(dp7["format"].dropna().unique())
        sel_fmt7 = st.pills(
            "Форматы для анализа", options=fmt_opts7,
            selection_mode="multi", default=fmt_opts7[:4] if len(fmt_opts7) >= 4 else fmt_opts7,
            key="env_fmt_pills",
        )
        dp7_f = dp7[dp7["format"].isin(sel_fmt7)] if sel_fmt7 else dp7
        ...
        bin_step = st.select_slider(
            ...
            value=200, key="heatmap_bin",
        )

        if sel_fmt7:
            price_min7 = int(dp7_f["price"].min() // bin_step * bin_step)
            price_max7 = int(dp7_f["price"].max() // bin_step * bin_step) + bin_step
            bins = range(price_min7, price_max7 + bin_step, bin_step)
            bin_labels = [f"{b}–{b+bin_step}" for b in bins]

            heat_rows = []
            for fmt in sel_fmt7:
                mkt_fmt = dp7_market[dp7_market["format"] == fmt]["price"]
                for b in bins:
                    cnt = int(((mkt_fmt >= b) & (mkt_fmt < b + bin_step)).sum())
                    heat_rows.append({"Формат": fmt, "Цена": b, "Позиций": cnt})
            heat_df = pd.DataFrame(heat_rows)
            heat_pivot = heat_df.pivot(index="Цена", columns="Формат", values="Позиций").fillna(0)
            ...
            # Нанесём цены КЕРАМИН как маркеры
            k_fmt_med = dp7_keramin[dp7_keramin["format"].isin(sel_fmt7)].groupby("format")["price"].median()
            for fmt, kprice in k_fmt_med.items():
                x_idx = sel_fmt7.index(fmt) if fmt in sel_fmt7 else None
        ...
        # ── 3. Ценовой пояс: сколько конкурентов рядом с КЕРАМИН ────────────
        ...
        band_pct = st.slider(
            "Ширина пояса ±%", 5, 50, 20, step=5, key="band_pct",
        )

        band_rows = []
        for _, krow in dp7_keramin.iterrows():
            kp = krow["price"]
            lo, hi = kp * (1 - band_pct / 100), kp * (1 + band_pct / 100)
            rivals = dp7_market[
                (dp7_market["format"] == krow["format"]) &
                (dp7_market["price"] >= lo) &
                (dp7_market["price"] <= hi)
            ]
            cheaper = rivals[rivals["price"] < kp]
            pricier = rivals[rivals["price"] >= kp]
            band_rows.append({
                "Позиция": krow.get("name", ""),
                "Формат":  krow["format"],
                "Цена КЕРАМИН": int(kp),
                f"Дешевле (в поясе ±{band_pct}%)": len(cheaper),
                f"Дороже (в поясе ±{band_pct}%)":  len(pricier),
                "Всего конкурентов": len(rivals),
            })

        band_df = pd.DataFrame(band_rows).sort_values(
            "Всего конкурентов", ascending=False
        ).reset_index(drop=True)

        fig_band = px.bar(
            band_df,
            x="Позиция",
            y=[f"Дешевле (в поясе ±{band_pct}%)", f"Дороже (в поясе ±{band_pct}%)"],
            barmode="stack",
            color_discrete_map={
                f"Дешевле (в поясе ±{band_pct}%)": "#2A9D8F",
                f"Дороже (в поясе ±{band_pct}%)":  "#E9C46A",
            },
        ...
        download_button(band_df, "price_band.xlsx", "Скачать Excel")
